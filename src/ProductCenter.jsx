...
function generatePdfReport() {
  ...
  const movements = [
    ...incomes.map((item) => ({ date: item.date || '', type: 'Ingreso', detail: item.source || item.type || item.description || item.note || 'Ingreso', amount: Number(item.amount || 0) })),
    ...expenses.map((item) => ({ date: item.date || '', type: 'Gasto', detail: item.category || item.note || 'Gasto', amount: -Number(item.amount || 0) }))
  ...
  function addPage() { doc.addPage(); addHeader(); return 44; }
  addHeader();
  let y = 44;
  const cardGap = 4;
  ...
  cards.forEach((card, index) => {
    ...
    doc.text(doc.splitTextToSize(card.value, cardWidth - 8), x + 4, y + 16);
  });
  y += 36;
  doc.setFont('helvetica', 'bold');
  doc.setFontSize(14);
  ...
  if (!movements.length) {
    ...
    movements.slice(0, 500).forEach((movement, index) => {
      ...
      if (y + rowHeight > pageHeight - 14) { y = addPage(); drawTableHeader(); }
      if (index % 2 === 0) { doc.setFillColor(252, 247, 246); doc.rect(margin, y, contentWidth, rowHeight, 'F'); }
