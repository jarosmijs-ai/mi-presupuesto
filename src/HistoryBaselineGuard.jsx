...
function updateBaselineUi() {
  const incomes = loadArray(INCOME_KEY).filter((item) => isWithinHistory(item.date));
  const expenses = loadArray(EXPENSE_KEY).filter((item) => isWithinHistory(item.date));
  const cumulativeBalance = sum(incomes) - sum(expenses);
...
export default function HistoryBaselineGuard() {
  useEffect(() => {
    const refresh = () => window.setTimeout(updateBaselineUi, 40);
    refresh();
